import { getExpenses } from "@/app/services/expenses/getExpenses";
                        ...
                        <tbody className="bg-white/50 dark:bg-zinc-800/40 divide-y divide-gray-100 dark:divide-gray-700/50">
                            {expenses.map((expense: ExpenseItem) => (
                                <tr key={expense.id} className="border-b last:border-none hover:bg-gray-50 dark:hover:bg-zinc-700/50">
                                    <td className="whitespace-nowrap pl-4 md:pl-6 pr-3 py-2 md:py-3 text-xs md:text-sm">{new Date(expense.date).toLocaleDateString("en-GB", { timeZone: "Asia/Dhaka" })}</td>
